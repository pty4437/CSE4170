...
loc_Material_Parameters loc_block;
...
GLint loc_block_effect, loc_block_frequency, loc_block_width;
...
int count = 0;
static int light4_flag = 0;
static int light1_flag = 0;
static int draw_block_flag = 0;
static int block_effect_flag = 0;

float block_frequency, block_width;
...
void initialize_block(void) {
	draw_block_flag = block_effect_flag = 0;

	block_frequency = 1.0f;
	block_width = 0.125;	
...
float clock_block = 0.0f;

float clock_light4 = 0.0f;
...
void prepare_floor(void) { // Draw coordinate axes.
	...
	//block 색깔
	material_block.ambient_color[0] = 0.15f;
	material_block.ambient_color[1] = 0.30f;
	material_block.ambient_color[2] = 0.12f;
	material_block.ambient_color[3] = 1.0f;
	//
	material_block.diffuse_color[0] = 0.0f;
	material_block.diffuse_color[1] = 0.0f;
	material_block.diffuse_color[2] = 0.0f;
	material_block.diffuse_color[3] = 1.0f;
	//
	material_block.specular_color[0] = 0.028281f;
	material_block.specular_color[1] = 0.355802f;
	material_block.specular_color[2] = 0.966065f;
	material_block.specular_color[3] = 1.0f;
	//
	material_block.specular_exponent = 51.2f;
	//
	material_block.emissive_color[0] = 0.0f;
	material_block.emissive_color[1] = 0.0f;
	material_block.emissive_color[2] = 0.0f;
	material_block.emissive_color[3] = 1.0f;
...
void set_material_block(void) {
	// assume ShaderProgram_PS is used
	glUniform4fv(loc_material.ambient_color, 1, material_block.ambient_color);
	glUniform4fv(loc_material.diffuse_color, 1, material_block.diffuse_color);
	glUniform4fv(loc_material.specular_color, 1, material_block.specular_color);
	glUniform1f(loc_material.specular_exponent, material_block.specular_exponent);
	glUniform4fv(loc_material.emissive_color, 1, material_block.emissive_color);
}
...
void prepare_ben(void) {
	...
	My_glTexImage2D_from_file("Data/static_objects/ben.png");
...
void prepare_tank(void) {
	...
	My_glTexImage2D_from_file("Data/static_objects/tank.png");
...
void display(void) {
	...
	if (draw_block_flag) {
		set_material_block();
		glUniform1i(loc_texture, 30);
		ModelViewMatrix = glm::translate(ViewMatrix, glm::vec3(500.0f, 200.0f, 250.0f));
		//	ModelViewMatrix = glm::scale(ModelViewMatrix, glm::vec3(500.0f, 500.0f, 500.0f));
		ModelViewMatrix = glm::rotate(ModelViewMatrix, -45.0f * TO_RADIAN, glm::vec3(1.0f, 0.0f, 0.0f));
		...
		if(clock_block < 300)
			glUniform1i(loc_block_effect, block_effect_flag);
		else
			glUniform1i(loc_block_effect, 0);
		draw_block();
		glUniform1i(loc_block_effect, 0);
		...
		if(clock_block >= 300 && clock_block < 600)
			glUniform1i(loc_block_effect, block_effect_flag);
		else
			glUniform1i(loc_block_effect, 0);
		draw_block();
		...
		set_material_block();
		glUniform1i(loc_texture, 30);
		ModelViewMatrix = glm::translate(ViewMatrix, glm::vec3(500.0f, 50.0f, 500.0f));
		//	ModelViewMatrix = glm::scale(ModelViewMatrix, glm::vec3(500.0f, 500.0f, 500.0f));
		...
		if (clock_block >= 600 && clock_block < 900)
			glUniform1i(loc_block_effect, block_effect_flag);
		else
			glUniform1i(loc_block_effect, 0);
		draw_block();
	...
	if (light4_flag == 1) {
		ModelViewMatrix = glm::rotate(ViewMatrix, -1.0f * clock_light4 * TO_RADIAN, glm::vec3(0.0f, 1.0f, 0.0f));
		ModelViewMatrix = glm::scale(ModelViewMatrix, glm::vec3(1.0f * (1 / clock_light4), 1.0f * (1 / clock_light4), 1.0f * (1 / clock_light4)));

		glUseProgram(h_ShaderProgram_TXPS);
		position_EC = ModelViewMatrix * glm::vec4(light[4].position[0], light[4].position[1],
			light[4].position[2], light[4].position[3]);
		glUniform4fv(loc_light[4].position, 1, &position_EC[0]);
		direction_EC = glm::mat3(ModelViewMatrix) * glm::vec3(light[4].spot_direction[0],
			light[4].spot_direction[1], light[4].spot_direction[2]);
		...
		glUseProgram(h_ShaderProgram_TXPS);
		position_EC = ViewMatrix * glm::vec4(light[4].position[0], light[4].position[1],
			light[4].position[2], light[4].position[3]);
		glUniform4fv(loc_light[4].position, 1, &position_EC[0]);
		direction_EC = glm::mat3(ViewMatrix) * glm::vec3(light[4].spot_direction[0],
			light[4].spot_direction[1], light[4].spot_direction[2]);
		glUniform3fv(loc_light[4].spot_direction, 1, &direction_EC[0]);
		glUseProgram(0);
	}
	...
	direction_EC = glm::mat3(ModelViewMatrix) * glm::vec3(light[3].spot_direction[0],
...
void timer_scene(int value) {
	...
	clock_light4 = timestamp_scene % 1000;

	clock_block = timestamp_scene % 900;
...
void motion(int x, int y) {
	...
	if (motion_flag == -1) return;
		...
		if (y > centerY) {
			ViewMatrix = glm::translate(ViewMatrix, glm::vec3(0.0f, -100.0f, 0.0f));
...
void initialize_lights_and_material(void) { // follow OpenGL conventions for initialization
	...
	glUniform4f(loc_block.ambient_color, 0.1f, 0.1f, 0.1f, 1.0f);
	glUniform4f(loc_block.diffuse_color, 0.1f, 0.1f, 0.1f, 1.0f);
	glUniform4f(loc_block.specular_color, 0.1f, 0.1f, 0.1f, 1.0f);
	glUniform4f(loc_block.emissive_color, 0.1f, 0.1f, 0.1f, 1.0f);
	glUniform1f(loc_block.specular_exponent, 0.0f);


	glUniform1i(loc_block_effect, 0);
	glUniform1f(loc_block_frequency, 1.0f);
	glUniform1f(loc_block_width, 0.125f);
...
void initialize_OpenGL(void) {
	...
	ProjectionMatrix2 = glm::mat4(1.0f);
...
void keyboard(unsigned char key, int x, int y) {
	...
	case '3':
		...
		return;
	case '4':
		light4_flag = 1 - light4_flag;
		glUseProgram(h_ShaderProgram_TXPS);
		glUniform1i(light4_effect, light4_flag);
		glUseProgram(0);
		glutPostRedisplay();
		break;
	case '5':
		draw_block_flag = 1 - draw_block_flag;
		glUseProgram(h_ShaderProgram_TXPS);
		glUniform1i(loc_block_effect, draw_block_flag);
		glUseProgram(0);
		glutPostRedisplay();
		break;
	case '6':
		light1_flag = 1 - light1_flag;
		glUseProgram(h_ShaderProgram_TXPS);
		glUniform1i(light1_effect, light1_flag);
		glUseProgram(0);
		glutPostRedisplay();
		break;
	case 'e':
		if (draw_block_flag) {
			block_effect_flag = 1 - block_effect_flag;
			glutPostRedisplay();
		}
		break;
	case 'p':
	...
	case 'c':
		...
		if (camera5_flag == 1) {
			...
			glUseProgram(h_ShaderProgram_TXPS);
			position_EC = ViewMatrix * glm::vec4(0,0,0,1);
			glUniform4fv(loc_light[2].position, 1, &position_EC[0]);
			direction_EC = glm::mat3(ViewMatrix) * glm::vec3(light[2].spot_direction[0],
				light[2].spot_direction[1], light[2].spot_direction[2]);
			glUniform3fv(loc_light[2].spot_direction, 1, &direction_EC[0]);
...
void prepare_shader_program(void) {
	...
	light4_effect = glGetUniformLocation(h_ShaderProgram_TXPS, "u_4_effect");
	light1_effect = glGetUniformLocation(h_ShaderProgram_TXPS, "u_1_effect");

	loc_block_effect = glGetUniformLocation(h_ShaderProgram_TXPS, "block_effect");
	loc_block_frequency = glGetUniformLocation(h_ShaderProgram_TXPS, "block_frequency");
	loc_block_width = glGetUniformLocation(h_ShaderProgram_TXPS, "block_width");

	
	loc_block.ambient_color = glGetUniformLocation(h_ShaderProgram_TXPS, "u_block.ambient_color");
	loc_block.diffuse_color = glGetUniformLocation(h_ShaderProgram_TXPS, "u_block.diffuse_color");
	loc_block.specular_color = glGetUniformLocation(h_ShaderProgram_TXPS, "u_block.specular_color");
	loc_block.emissive_color = glGetUniformLocation(h_ShaderProgram_TXPS, "u_block.emissive_color");
	loc_block.specular_exponent = glGetUniformLocation(h_ShaderProgram_TXPS, "u_block.specular_exponent");
...
void set_up_scene_lights(void) {
	...
	light[1].light_on = 1;
	light[1].position[0] = 1.0f; light[1].position[1] = 1000.0f; // spot light position in WC
	light[1].position[2] = 1.0f; light[1].position[3] = 1.0f;
	...
	light[2].light_on = 1;
	light[2].position[0] = 0.0f; light[2].position[1] = 1000.0f; // spot light position in WC
	...
	light[3].light_on = 1;
	light[3].position[0] = 1.0f; light[3].position[1] = 300.0f; // spot light position in WC
	light[3].position[2] = 1.0f; light[3].position[3] = 1.0f;

	light[3].ambient_color[0] = 0.152f; light[3].ambient_color[1] = 0.152f;
	light[3].ambient_color[2] = 0.152f; light[3].ambient_color[3] = 1.0f;

	light[3].diffuse_color[0] = 1.0f; light[3].diffuse_color[1] = 0.3f;
	light[3].diffuse_color[2] = 0.3f; light[3].diffuse_color[3] = 1.0f;

	light[3].specular_color[0] = 1.0f; light[3].specular_color[1] = 0.3f;
	...
	light[4].light_on = 1;
	light[4].position[0] = 200.0f; light[4].position[1] = 100.0f; // spot light position in WC
	light[4].position[2] = 200.0f; light[4].position[3] = 1.0f;

	light[4].ambient_color[0] = 0.152f; light[4].ambient_color[1] = 0.152f;
	light[4].ambient_color[2] = 0.152f; light[4].ambient_color[3] = 1.0f;

	light[4].diffuse_color[0] = 0.17f; light[4].diffuse_color[1] = 0.255f;
	light[4].diffuse_color[2] = 0.30f; light[4].diffuse_color[3] = 1.0f;

	light[4].specular_color[0] = 1.0f; light[4].specular_color[1] = 1.0f;
	light[4].specular_color[2] = 1.0f; light[4].specular_color[3] = 1.0f;

	light[4].spot_direction[0] = -60.0f; light[4].spot_direction[1] = -1.0f; // spot light direction in WC
	light[4].spot_direction[2] = -60.0f;
	light[4].spot_cutoff_angle = 20.0f;
	light[4].spot_exponent = 1.0f;
	...
	glUniform1i(loc_light[3].light_on, light[3].light_on);
	// need to supply position in EC for shadingloc_light[0]
	glm::vec4 position_EC4 = ViewMatrix * glm::vec4(light[3].position[0], light[3].position[1],
		light[3].position[2], light[3].position[3]);
	glUniform4fv(loc_light[3].position, 1, &position_EC4[0]);
	...
	// note that the viewing transform is a rigid body transform
	// thus transpose(inverse(mat3(ViewMatrix)) = mat3(ViewMatrix)
	glm::vec3 direction_EC4 = glm::mat3(ViewMatrix) * glm::vec3(light[3].spot_direction[0], light[3].spot_direction[1],
		light[3].spot_direction[2]);
	glUniform3fv(loc_light[3].spot_direction, 1, &direction_EC4[0]);
	...
	// need to supply position in EC for shadingloc_light[0]
	glm::vec4 position_EC5 = ViewMatrix * glm::vec4(light[4].position[0], light[4].position[1],
		light[4].position[2], light[4].position[3]);
	glUniform4fv(loc_light[4].position, 1, &position_EC5[0]);
	...
	// note that the viewing transform is a rigid body transform
	// thus transpose(inverse(mat3(ViewMatrix)) = mat3(ViewMatrix)
	glm::vec3 direction_EC5 = glm::mat3(ViewMatrix) * glm::vec3(light[4].spot_direction[0], light[4].spot_direction[1],
		light[4].spot_direction[2]);
	glUniform3fv(loc_light[4].spot_direction, 1, &direction_EC5[0]);
	...
	// need to supply position in EC for shading
	glm::vec4 position_EC2 = ViewMatrix2 * glm::vec4(light2[1].position[0], light2[1].position[1],
	...
	// need to supply direction in EC for shading in this example shader
	glm::vec3 direction_EC2 = glm::mat3(ViewMatrix2) * glm::vec3(light2[1].spot_direction[0], light2[1].spot_direction[1],
